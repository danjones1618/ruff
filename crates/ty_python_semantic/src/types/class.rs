...
impl<'db> ClassLiteral<'db> {
    ...
    pub fn is_typed_dict(self, db: &'db dyn Db) -> bool {
        self.as_stmt().is_some_and(|stmt| stmt.is_typed_dict(db))
    }
    ...
    /// Functional classes cannot be `TypedDicts`, so this delegates to the Stmt variant.
    ...
    pub(super) fn own_synthesized_member(
        ...
        if self.total_ordering(db) && matches!(name, "__lt__" | "__le__" | "__gt__" | "__ge__") {
            ...
            let has_ordering_method = self
                ...
                .filter(|class| !class.class_literal(db).is_known(db, KnownClass::Object))
                .any(|class| {
                    class
                        .class_literal(db)
                        .as_stmt()
                        .is_some_and(|stmt| stmt.has_own_ordering_method(db))
                });
